...
//      
//      // Fetch by ID
//    let fetchedID = await database.fetch(for: .descriptor(predicate: #Predicate<Parent>{ $0.id == parentID}) { parent in
//        parent.id
//      }
//      
...
//
  @Test internal func testFetchByIDs() async throws {
      ...
      // Fetch by IDs
    let fetchedIDs = await database.fetch(for: .descriptor(predicate:  #Predicate<Parent>{ parentIDs.contains($0.id)})) { parents in
        parents.map(\.id)
      }
...
//      
//      // Fetch by predicate
//      let fetchedID = await database.fetch(for: .predicate(#Predicate<Parent> { parent in
//        parent.id == parentID
//      })) { parents in
//        parents.first?.id
//      }
...
//    
//      // Fetch by value
//    let fetchedID = await database.get(for: .descriptor(predicate: #Predicate<Parent>{ $0.id == parentID})) { parent in
//        parent.id
//      }
//      
//      #expect(fetchedID == parentID)
