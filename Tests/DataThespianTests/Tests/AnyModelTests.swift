...
internal struct AnyModelTests {
  @Test internal func testAnyModelInitWithPersistentIdentifier() throws {
    #if canImport(SwiftData)
  ...
  @Test internal func testAnyModelInitWithPersistentModel() throws {
    #if canImport(SwiftData)
      let database = try TestingDatabase(for: Parent.self, Child.self)
      try await database.withModelContext { context in
  ...
  @Test internal func testTypeErasureFromModel() throws {
    #if canImport(SwiftData)
      let database = try TestingDatabase(for: Parent.self, Child.self)
      try await database.withModelContext { context in
        // Create and save a parent
  ...
  @Test internal func testCreateTypedModelFromAnyModel() throws {
    #if canImport(SwiftData)
      let database = try TestingDatabase(for: Parent.self, Child.self)
      try await database.withModelContext { context in
        let parent = Parent(id: UUID())
